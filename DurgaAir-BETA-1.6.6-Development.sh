#!/bin/sh
## Durga-Air v1.6.6 Beta by NoeticBSD | Email: noeticbsd@gmail.com | IRC: noetic@irc.freenode.net #freebsd | Github http://github.com/NoeticBSD ##
clear
...
-secured)
	...
	case $ans1 in
		...
		*)
			...
			wpa_supplicant -i$device -c /$HOME/.wpa_supplicant.conf &
	esac
;;
-saved)
	sudo wpa_supplicant -i$device -c /$HOME/.wpa_supplicant.conf &
;;
-NS)
	if [ ! -z $2 ] ; then
